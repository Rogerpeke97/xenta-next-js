...
const FormSignUp = ({ onSignUpSuccess }: { onSignUpSuccess: () => void }) => {
  ...
  const { isLoading, refetch } = useSignInUser({
    username: form.email,
  ...
  return (
    <form className="flex flex-col w-full pop-in" onSubmit={signUp}>
      <div className="px-6 flex flex-col items-center justify-center">
        <FormField isValid={form.isValidUser} value={form.name} onChange={(e) => onInputChange((e.target as HTMLTextAreaElement).value, 'name')}
